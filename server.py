    ...
    try:
        ...
        c.executescript("""
            CREATE TABLE IF NOT EXISTS nodes (
                ...
                os TEXT, cpu_type TEXT, last_seen TEXT,
                status TEXT DEFAULT 'ACTIVE'
            );
            CREATE TABLE IF NOT EXISTS metrics (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                node_id TEXT NOT NULL, timestamp TEXT NOT NULL,
                cpu REAL, memory REAL, disk REAL, uptime INTEGER,
                created_at TEXT DEFAULT (datetime('now'))
            );
            CREATE TABLE IF NOT EXISTS services (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                metric_id INTEGER, node_id TEXT,
                service_name TEXT, status TEXT, timestamp TEXT
            );
            CREATE TABLE IF NOT EXISTS ports (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                metric_id INTEGER, node_id TEXT,
                port INTEGER, status TEXT, timestamp TEXT
            );
            CREATE TABLE IF NOT EXISTS alerts (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                node_id TEXT, message TEXT,
...
class ClientHandler:
    ...
    def process_message(self, raw_msg):
        ...
        if not self.validate_message(data):
            ...
            return
        self.last_seen = time.time()
        if data.get("type", "METRICS") == "METRICS":
            self.node_id = data.get("node")
            self.save_metrics(data)
            self.server.update_node_display(data)

    def validate_message(self, data):
        return all(k in data for k in ["node", "timestamp", "cpu", "memory", "disk", "uptime"])
        ...
        try:
            ...
            self.pool.execute("""
                ...
                ON CONFLICT(node_id) DO UPDATE SET
                    ...
                    os=excluded.os, cpu_type=excluded.cpu_type
            ...
            if alert_list:
                send_alert_email(node_id, alert_list)
    ...
    def mark_node_down(self):
        if self.node_id:
            self.pool.execute("UPDATE nodes SET status='DOWN' WHERE node_id=?", (self.node_id,))
            self.pool.execute(
                "INSERT INTO logs (level, message) VALUES ('DOWN', ?)",
                (f"Nœud {self.node_id} en panne (timeout {TIMEOUT_INACTIVE}s)",)
            )
...
class SupervisionServer:
    ...
    def get_recent_metrics(self, node_id, limit=20):
        return self.pool.fetchall(
            "SELECT * FROM metrics WHERE node_id=? ORDER BY id DESC LIMIT ?", (node_id, limit))

    def get_recent_alerts(self, limit=50):
        return self.pool.fetchall("SELECT * FROM alerts ORDER BY id DESC LIMIT ?", (limit,))

    def get_logs(self, limit=100):
        return self.pool.fetchall("SELECT * FROM logs ORDER BY id DESC LIMIT ?", (limit,))
...
class SupervisionGUI:
    ...
    def _build_tab_admin(self):
        ...
        tk.Button(form, text="▶  Envoyer la commande", command=self.send_command,
                  bg="#a6e3a1", fg="#1e1e2e", font=("Helvetica", 11, "bold"),
                  padx=20, pady=8).grid(row=3, columnspan=2, pady=15)
        self.cmd_result = tk.Label(frame, text="", bg="#1e1e2e", fg="#89b4fa", font=("Helvetica", 10))
    ...
    def _refresh_nodes_tab(self):
        ...
        for row in rows:
            ...
            self.tree_nodes.insert("", "end", values=(
                ...
                nd.get("cpu","-"), nd.get("memory","-"),
                nd.get("disk","-"), nd.get("uptime","-"),
    ...
    def _refresh_alerts(self):
        ...
        for a in alerts:
            self.tree_alerts.insert("", "end", values=(
                a["id"], a["node_id"], a["message"], a["timestamp"]), tags=("alert_row",))
